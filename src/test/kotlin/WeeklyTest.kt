...
@Test
fun `test one day`() {
    assertThat(weeklySchedule(
            createSchedule("2017-07-19", "2017-07-26", 2)))
...
private fun `test multiple days across months`() {
    assertThat(weeklySchedule(
            createSchedule("2017-07-28", "2017-08-03", 3,5)))
            .containsExactlyInAnyOrder(
                    parse("2017-07-28"),
                    parse("2017-08-02"))
}
...
private fun createSchedule(startDate: String, endDate: String, vararg nDay: Int) = Schedule(
        parse(startDate), parse(endDate), ScheduleType.WEEKLY, nDay.toList())
